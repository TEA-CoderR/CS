...
int add_task(threadpool_t *pool, void *(*fun)(void *), void *arg){
	...
	printf("add_task pos:%d, %d------------------------%d\n", pool->tail, *(int*)pool->task_queue[pool->tail].arg, *(int*)arg);
	fflush(stdout);
	++pool->qlen;
	/* coda non vuota piu, prende un task e esegue*/
	// for (int i = pool->head + 1; i < pool->qlen; ++i)
	// {
	// 	printf("dopo add queue[%d]:%d\n", i, *(int*)(pool->task_queue[i].arg));
	// 	printf("dopo add queue[%d]:%p\n", i, pool->task_queue[i].arg);
	// }
	//printf("added un task, qlen: %d\n", pool->qlen);
	SIGNAL_RETURN(&pool->not_empty, -1);/* notifica un worker a prendere il task*/
	//BROADCAST_RETURN(&pool->not_empty, -1);/*broadcast all workers a competere il task*/
	UNLOCK_RETURN(&pool->mtx, -1);
	return 0;
}
 ...
 */
int pop_task(threadpool_t *pool, taskfun_t *task){
	if(!pool || !task) return -1;
	pool->head = (pool->head + 1) % pool->queue_size;
	task->fun = pool->task_queue[pool->head].fun;
	task->arg = pool->task_queue[pool->head].arg;
	printf("pop un task, pos :%d, arg:%d\n", pool->head, *(int*)task->arg);
	--pool->qlen;
	return 0;
}
...
void *worker(void *thread_pool){
	...
	while(true){
		...
		if(pool->shutdown && pool->qlen == 0){
			UNLOCK(&pool->mtx);
			printf("%d exit because shutdown\n", (unsigned int)pthread_self());
			pthread_exit(NULL);
		}
		/* coda non vuota piu, prende un task e esegue*/
		// for (int i = pool->head + 1; i < pool->qlen; ++i)
		...
		task.fun = pool->task_queue[pool->head].fun;
		task.arg = pool->task_queue[pool->head].arg;
		printf("pop un task, pos :%d, %d-------------------arg:%d\n", pool->head, *(int*)pool->task_queue[pool->head].arg, *(int*)task.arg);
		//printf("%d removed un task, qlen: %d\n", (unsigned int)pthread_self(), pool->qlen);
		--pool->qlen;
...
int removen_worker(threadpool_t *pool, int n){
	...
	pool->exiting = (n >= pool->live_thr_num) ? (pool->live_thr_num - 1) : n;
	UNLOCK_RETURN(&pool->mtx, -1);
	return 0;
}
...
static void *auto_monitor(void *thread_pool){
	...
	while(true){
		...
		printf("monitor: alive:%d busy:%d qlen:%d\n", pool->live_thr_num, pool->busy_thr_num, pool->qlen);
