 ...
 * @return 0 On successo, return -1 On error.
 */
static countfile_t *packing_task_arg(char *dirname, char *filename, struct stat *info, int ssock);
...
int run_master(int argc, char *argv[], sigset_t *mask, struct sockaddr_un *sa){
	...
	if(!pool){
		cleanall(NULL, NULL, 0, sig_pipe, usr1_pipe, usr2_pipe);
		return -1;/* error*/
	}
	...
	for (int i = 0; i < n_sock; ++i)
	{
		...
		while(connect(ssock[i], (struct sockaddr*)sa, sizeof(*sa)) == -1){
			if(errno = ENOENT) sleep(1);
				...
				cleanall(pool, ssock, i, sig_pipe, usr1_pipe, usr2_pipe);
				exit(EXIT_FAILURE);
			}
		}
		//printf("%d\n", ssock[i]);
		//DEBUG(1, "connected successo\n");
	}
	...
	while(argv[optind] != NULL){
		...
		countfile_t *cf = NULL;
		if((cf = packing_task_arg(NULL, argv[optind], &info, ssock[++num_task % n_sock])) == NULL){
			cleanall(pool, ssock, 4, sig_pipe, usr1_pipe, usr2_pipe);
			return -1;/* On error*/
		}
		...
		if(r == 1){/* checked pool gia' shutdown*/
			...
			freememory(cf->filepath);
			freememory(cf);
			cleanall(pool, ssock, 4, NULL, usr1_pipe, usr2_pipe);
			return 1;
		} 
	...
	if(dirname){
		...
		if(S_ISDIR(info.st_mode)){
			 ...
			 *  se ritorna 1 signfica threadpool gia' chiuso!!!
			 */
			r = parsing_dir(pool, dirname, ssock, n_sock, &num_task, delay);
			if(r == -1){
				cleanall(pool, ssock, 4, sig_pipe, usr1_pipe, usr2_pipe);
				return -1; /* On error*/
			}
...
static int cleanall(threadpool_t *pool, int ssock[], int n, int sig_pipe[], int usr1_pipe[], int usr2_pipe[]){
	...
	printf("master threadpool_destroy\n");
	if(ssock){
		for (int i = 0; i < n; ++i)
		{
			SYSCALL_EXIT(notused, close(ssock[i]), "close");
			printf("master closed ssock[i] :%d\n", ssock[i]);
		}
	}

	if(sig_pipe){
		printf("master start close sigpipe\n");
		SYSCALL_EXIT(notused, close(sig_pipe[0]), "close");
		SYSCALL_EXIT(notused, close(sig_pipe[1]), "close");
		printf("master closed sigpipe\n");
	}
	SYSCALL_EXIT(notused, close(usr1_pipe[0]), "close");
	SYSCALL_EXIT(notused, close(usr1_pipe[1]), "close");
	SYSCALL_EXIT(notused, close(usr2_pipe[0]), "close");
	SYSCALL_EXIT(notused, close(usr2_pipe[1]), "close");
	printf("master closed usrpipe\n");
...
static int parsing_dir(threadpool_t *pool, char *dirname, int ssock[], int n_sock, int *num_task, int delay){
	...
	while((errno = 0, file = readdir(dir)) != NULL){
		...
		countfile_t *cf = NULL;
		if((cf = packing_task_arg(dirname, file->d_name, &info, ssock[++(*num_task) % n_sock])) == NULL) return -1;/* On error*/

		if(S_ISDIR(info.st_mode)){
			int r = parsing_dir(pool, cf->filepath, ssock, n_sock, num_task, delay);
