...
void *worker(void *thread_pool){
	...
	while(true){
		...
		LOCK_RETURN(&pool->count_busy_num, -1);/* start work, busy_thr_num + 1*/
		++pool->busy_thr_num;
		UNLOCK_RETURN(&pool->count_busy_num, -1);

		(*task.fun)(task.arg);/* esegue task*/

		LOCK_RETURN(&pool->count_busy_num, -1);/* end work, busy_thr_num - 1*/
		--pool->busy_thr_num;
		UNLOCK_RETURN(&pool->count_busy_num, -1);
	}
}
...
/** 
 * incrementa un thread monitor nel monitors nel pool.
 * 
 * @param pool puntatore al thread_pool
 ...
 * @return 0 On successo, -1 On error.
 */
int add_monitor(threadpool_t *pool, void *(*fun)(void *), void *arg){
	do{
		LOCK_RETURN(&pool->mtx, -1);
		if(pool->shutdown){/*verifica se e' chiuso thread_pool*/
			UNLOCK_RETURN(&pool->mtx, -1);
		...
		if(pthread_create(&new_monitor->tid, NULL, fun, arg) != 0){
			perror("pthread_create");
			break;
		}
...
threadpool_create(int pool_size, int queue_size, ...){
	...
	do{
		...
		/* add un auto_monitor nel pool*/
		if(add_monitor(pool, auto_monitor, pool) == -1){
			perror("add_monitor");
			break;
		}
		...
		while((new_monitor = va_arg(list, taskfun_t*)) != NULL){
			if(add_monitor(pool, new_monitor->fun, new_monitor->arg) == -1){
				perror("add_monitor");
				return NULL;
			}
		}
...
int threadpool_destroy(threadpool_t *pool){
	...
	/* set shutdown 1*/              ////////fang wai mian zuo
	LOCK_RETURN(&pool->mtx, -1);
	pool->shutdown = true;
	UNLOCK_RETURN(&pool->mtx, -1);
