...
#define EC_NOT0(c,e)    \
    if(c != 0) {perror(e); exit(EXIT_FAILURE);}
// #define CREATE(s,m)     \
...
int main(int argc, char const *argv[])
{
	...
	if(pid1){// server
	    ...
	    EC_MINUS1(e, pthread_sigmask(SIG_BLOCK, &mask, &oldmask), "pthread_sigmask");
	    struct sigaction s;
	    memset(&s, 0 ,sizeof(s));
	    s.sa_handler = SIG_IGN;
	    EC_MINUS1(e, sigaction(SIGPIPE, &s, NULL), "sigaction");
	    /* reset oldset*/
	    //EC_MINUS1(e, pthread_sigmask(SIG_SETMASK, &oldmask, NULL), "pthread_sigmask");
//	    EC_MINUS1(e, pthread_sigmask(SIG_SETMASK, &mask, NULL), "pthread_sigmask");
		/* configure server*/
		...
		pthread_t shandler;
		if(pthread_create(&shandler, NULL, sighandler, &mask) != 0){
		...
		unlink(SOCKETNAME);
		if(pthread_join(shandler, NULL) != 0){
			fprintf(stderr, "FATAL ERROR, pthread_join\n");
			return -1;;
		}
		destroypool(pool);
		free(pool);
...
static void* sighandler(void *arg){
    ...
    // sigaddset(&mask, SIGTERM);
    // /* maschera i segnali prima di installare*/
	//EC_MINUS1(e, pthread_sigmask(SIG_SETMASK, &mask, &oldmask), "pthread_sigmask");
