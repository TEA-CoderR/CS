 ...
 * 
 * @return 0 On successo, return 1 se pool e' chiuso, On error return -1.
 */
...
void *worker(void *thread_pool){
	...
	while(true){
		...
		while(pool->qlen == 0 && !pool->shutdown){/*coda vuota*/
			...
			// /**svegliato, controlla pool se e' necessario terminare alcuni threads,
			//  * se si, thread termina se stesso e exiting - 1.*/
			// if(pool->exiting > 0){
			// 	update_state(pool, (unsigned int)pthread_self());
			// 	--pool->exiting;
...
int removen_worker(threadpool_t *pool, int n){
	...
	if(pool->shutdown == 1){
		UNLOCK_RETURN(&pool->mtx, -1);
		DEBUG(1, "jujue removen_worker");
		return -1;
	}
	/* al massimo termina live_thr_num -  threads*/
	pool->exiting = (n >= pool->live_thr_num) ? (pool->live_thr_num - 1) : n;
	UNLOCK_RETURN(&pool->mtx, -1);
	DEBUG(1, "chenggong removen_worker");
	return 0;
}
...
static void *auto_monitor(void *thread_pool){
	...
	while(true){
		...
		/* non causa deadlock mai, perche prende mtx first sempre*/
 ...
 * 
 * @param pool puntatore al thread_pool
 * @param fun puntarore a funzione monitor da chiamare
 * @param arg puntatore a argomento passato della funzione
 * 
 * @return 0 On successo, -1 On error.
 */
...
/**
 * crea e inizialliza un threadpool.
 * 
...
int threadpool_destroy(threadpool_t *pool){
	...
	//pool->exiting = pool->live_thr_num;
	...
	while(pool->monitors != NULL){
		monitor_t *tmp_monitors = pool->monitors;
		pool->monitors = pool->monitors->next;
		if(pthread_join(tmp_monitors->tid, NULL) != 0){
			perror("join");
			return -1;
		}
		free(tmp_monitors);
...
int main(int argc, char const *argv[])
{
	...
	threadpool_t *pool = threadpool_create(4, 8, &task, NULL);
	int q = 2, w = 4, r = 6, t = 9;
	add_task(pool, play, &q);
	add_task(pool, play, &w);
	removen_worker(pool, 1);
	add_task(pool, play, &r);
	addn_worker(pool, 2);

	threadpool_close(pool);
	
	add_task(pool, play, &t);
	removen_worker(pool, 1);
	addn_worker(pool, 2);
