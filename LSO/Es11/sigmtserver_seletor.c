			...
			if(select(fd_num + 1, &tmpset, NULL, NULL, NULL) == -1){
				...
				for (int fd = 0; fd <= fd_num; ++fd)
				{
					if(FD_ISSET(fd, &tmpset)){
						...
						// associa un signal_pipe, se e' attivo, close server
						/** associa un request_pipe, se e' attivo, read un fd e registra nella set
						 * anche un tid di thread, e join questo tid
						 */
					}
				}
			}
...
static void* woker(void *arg){
	...
	if(nread == 0 ){
		FD_CLR(csock, &set);
		if(csock == fd_num) fd_num = update(csock, &set);
		//EC_MINUS1(e, close(csock), "close");
	}
	else{
		buf[nread] = '\0';
...
static void* request(void *arg){
	...
	char *filename = strndup(re->filename, strlen(re->filename));
	char *buf = NULL;
	FILE *fin = NULL;
	EC_NULL((buf = calloc(sizeof(char), N)), "calloc");
