...
#endif
#ifndef NWORKERATEXIT
#define NWORKERATEXIT "nworkeratexit.txt"
#endif
...
int removen_worker(threadpool_t *pool, int n){
	...
	/** setta il thr_num da terminare, se live_thr_num e' 1, ignora la operazione, 
	 * invece al massimo termina live_thr_num threads - 1*/
	if(pool->live_thr_num > 1) pool->exiting = (n >= pool->live_thr_num) ? (pool->live_thr_num - 1) : n;
	
	/* notifica i thread in attesa(SE C'E) a terminare*/
	for (int i = 0; i < pool->exiting; ++i)
	{
...
int threadpool_destroy(threadpool_t *pool){
	...
	/* se non e' shutdown, set threadpool shutdown 1*/ 
	if(!pool->shutdown) threadpool_close(pool);
